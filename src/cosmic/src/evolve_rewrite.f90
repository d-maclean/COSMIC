...
! flags & settings
  ...
  real(dp):: bcm(max_loop,n_col_bcm)
...
! star,mass,tb,ecc,z,tphysf,
  ...
  subroutine evolv2(kstar, mass, porb, ecc, z,tphysf,&
    ...
    logical:: output
    logical:: switchedCE,disrupt,binary_just_destroyed,finished
    integer ierr
    ...
    if(output) write(*,*) 'finished with declarations'
    ! initialize stellar components
    if (using_METISSE == 1) then
      call initialize_front_end('cosmic')
      if(output) write(*,*) 'finished initializing frontend'
    endif
    call zcnsts(z,zpars)
    if(output) write(*,*) 'finished with zcnsts'
    if (using_METISSE == 1) then
    ...
    if(output) write(*,*) 'finished allocating tracks'
    kmin = 1
    kmax = 2
    sgl = .false.
    rlof = .false.
...
! output logging
    ...
    evolv_main: do while(.not.finished.and.loop<max_loop)
      ...
      evolv_detached: do while(.not.rlof)
        if(output)write(*,*) "in `evolv_detached`"
        if (xi == 0) then
          ospin(1) = 1.d-10
          ospin(2) = 1.d-10
        endif
...
! Radiative damping (Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329)
        ...
        if(.not.sgl) then
          ...
          if(ecc>=1.d0) then
            binary_just_destroyed = .true.
            exit evolv_detached
          endif
          !if(ecc>=1.d0) goto 135 ! TODO remove goto
      ...
      evolv_rlof: do while(rlof)
        ...
        elseif(&
          ((kstar(j1)==3.or.kstar(j1)==5.or.kstar(j1)==6.or.&
          kstar(j1)==8.or.kstar(j1)==9).and.(q(j1)>qc.or.&
          radx(j1)<=radc(j1))).or.(kstar(j1)==2.and.&
          q(j1)>qc).or.(kstar(j1)==4.and.q(j1)>qc))then
          ...
          evolve_type = end_CE ! end common envelope
          mass1_bpp = mass(1)
          mass2_bpp = mass(2)
        ...
        elseif(kstar(j1)>=10.and.kstar(j1)<=12.and.q(j1)>qc)then
        ...
        elseif(kstar(j1)==14)then
          ...
          do k=kmin,kmax
            ...
            if(neta>tiny .and. kstar(k)<15)then
              ...
              if(dmt(3-k)==dme/porb) supedd = .true.
              beta = betahold
            else
              dmr(k) = 0.d0
              dmt(3-k) = 0.d0
          ...
          elseif(kstar(j1)<=6.and.(kstar(j2)>=10.and.kstar(j2)<=12))then
            ! White dwarf secondary.

            if(dm1/porb<2.71d-07)then
              if(dm1/porb<1.03d-07)then
          ...
          do k=kmin,kmax
            ...
            elseif(htpmb==1)then
              if(ST_cr<=0.and.(mass(k)>0.35d0.and.kstar(k)<10.and.&
                menv(k)>0.0d0))then
                ! Ivanova & Taam (2003)
                if (ospin(k)<=wx) djmb = kw3 * rad(k)**4.0d0 * (ospin(k)/wsun)**3.0d0
                if (ospin(k)>wx) djmb = kw3 * rad(k)**4.0d0 * (ospin(k)/wsun)**1.3d0 * (wx/wsun)**1.7d0
                djspint(k) = djspint(k) + djmb
              elseif(ST_cr>0.and.(menv(k)>0.d0.and.&
                ...
                ((kstar(k)==3).or.(kstar(k)==5).or.(kstar(k)==6)))))then
            ...
            if(kstar(k)==13.and.pulsar>0)then
              if(bdecayfac==0)then
                ...
                if(B_0(k)==0.d0)then
                  B(k) = 0.d0
                elseif((tphys-epoch(k)-tacc(k))<tiny.and.bacc(k)==0.d0)then
                  B(k) = B_0(k) + Bbot
                elseif((tphys-epoch(k)-tacc(k))<tiny)then
                  B(k) = B_0(k)/(1.d0 + (bacc(k)/1.0d-6)) + Bbot
                elseif(bacc(k)==0.d0)then
          ...
          do k=kmin,kmax
            ...
            if(((kstar(k)<=9.and.rad(k)>=0.01d0*rol(k)).or.&
              ...
              if(ST_cr<=0.and.((kstar(k)==1.and.mass(k)>=1.25d0).or.&
                ...
                tcqr = f*q(3-k)*raa6
                rg2 = k2str(k)
              elseif(ST_cr>0.and.((kstar(k)<=1.and.mass(k)>&
                convradcomp(k)).or.(kstar(k)==2.and.teff(k)>&
                convradcomp(k)).or.(kstar(k)==4.and.teff(k)>&
                convradcomp(k)).or.(kstar(k)==9.and.mass(k)>&
                convradcomp(k)).or.(kstar(k)==7).or.(kstar(k)==8)))then
                ! Radiative damping (Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329).
        ...
        if(kstar(j2)<=2.or.kstar(j2)==7)then
          ...
          elseif((mass(j2)<0.35d0.or.mass(j2)>1.25d0).and.&
            kstar(j2).ne.7)then
        ...
        if(tphys>=tphysf.and..not.inspiral) then
          finished = .true.
          exit evolv_rlof
      ...
      evolv_contact: if(cntct)then
        ! Contact system
        if(output)write(*,*) 'In `evolv_contact`'
        coel = .true.
        binstate = 1
        CALL CONCATKSTARS(kstar(j1), kstar(j2), mergertype)
        ...
        elseif(kstar(j2)>=2.and.kstar(j2)<=9.and.&
          kstar(j2).ne.7)then
          if(j1==1)then
            switchedCE = .true.
          else
          ...
          if(j2==2.and.kcomp1==13.and.kstar(j1)==&
            15.and.kstar(j2)==13)then !PK.
            ! In CE the NS got switched around. Do same to formation.
      ...
      if(binary_just_destroyed)then
        sgl=.true.
        binary_just_destroyed=.false.
        ...
        if(kstar(1).ne.15.or.kstar(2).ne.15)then
          ...
          elseif(ecc>1.d0)then
            ...
            evolve_type = no_remnant_leftover
            porb = 0.d0
            sep = 0.d0
            teff1 = 1000.d0*((1130.d0*lumin(1)/&
              (rad(1)**2.d0))**(1.d0/4.d0))
        ...
        if(kstar(1)==13.and.ecc<=1.d0.and.pulsar>0.and.notamerger==0)then
          age = 0.d0
          epoch(1) = tphys
        endif
        if(kstar(2)==13.and.ecc<=1.d0.and.pulsar>0.and.notamerger==0)then
          age = 0.d0
          epoch(2) = tphys
        endif
      ...
      evolv_finished:if(finished)then
        if(output)write(*,*) "in `finished`"
        if(com)then
          com = .false.
        else
          mass1_bpp = mass(1)
          ...
          elseif(kstar(1)==15.and.kstar(2)==15)then
            ...
            evolve_type = final_state
            !added by PA for systems that stop evolving halfway
            if(iter>=loop) evolve_type = error_rlof_timeout
            if (using_METISSE==1) then
              call check_error(ierr)
              if (ierr>0) evolve_type = error_METISSE_error
            end if
            rrl1 = rad(1)/rol(1)
            rrl2 = rad(2)/rol(2)
            teff1 = 1000.d0*((1130.d0*lumin(1)/&
