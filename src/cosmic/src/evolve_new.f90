...
contains
  ...
  SUBROUTINE evolv2(kstar,mass,tb,ecc,z,tphysf,&
    ...
    !f2py intent(in) ecc
    !f2py intent(in) z
    !f2py intent(in) tphysf
    !f2py intent(in) dtp
    !f2py intent(in) mass0
    !f2py intent(in) rad
    !f2py intent(in) lumin
    !f2py intent(in) massc
    !f2py intent(in) radc
    !f2py intent(in) menv
    !f2py intent(in) renv
    !f2py intent(in) ospin
    !f2py intent(in) B_0
    !f2py intent(in) bacc
    !f2py intent(in) tacc
    !f2py intent(in) epoch
    !f2py intent(in) tms
    ...
    !      if(id1_pass==1)then!.and.tphysf>17.50d0.and.
    !*     &   tphysf<17.6d0)then
    !         output = .true.
    !      endif
    ...
    !      idum1 = INT(sep*100)
    if(idum1>0.and.using_cmc==0) idum1 = -idum1

    ! Set the collision matrix.
    err = 0
    if(using_cmc==0)then
      if(using_METISSE==1) CALL initialize_front_end('cosmic')
      !      for SSE path_to_tracks and path_to_he_tracks are empty ('')
      CALL zcnsts(z,zpars)
      if(output)write(*,*) 'zcnsts:', zpars
    ...
    if(mt2<tiny.or.tb<=0.d0)then
      ...
      oorb = 0.d0
      jorb = 0.d0
      if(kstar(1).ne.14.d0.or.using_cmc==0) bhspin(1) = 0.d0
      if(kstar(2).ne.14.d0.or.using_cmc==0) bhspin(2) = 0.d0
    ...
    do 500 , k = kmin,kmax
      ...
      tms(k) = tm
      tbgb(k) = tscls(1)
      if(tphys<tiny.and.ospin(k)<=0.001d0)then
...
500 continue
    ...
    if(dtp<=0.d0)then
      ...
      isave = .false.
      tsave = tphysf
    elseif(dtp>tphysf)then
      isave = .false.
      tsave = tphysf
    endif
    if(tphys>=tphysf) goto 140
...
4   iter = 0
    ...
    j1 = 1
    j2 = 2
    if(kstar(1)>=10.and.kstar(1)<=14) dtmi(1) = 0.01d0
    if(kstar(2)>=10.and.kstar(2)<=14) dtmi(2) = 0.01d0
    dm1 = 0.d0
    dm2 = 0.d0
5   kw1 = kstar(1)
    kw2 = kstar(2)
    ...
    if(intpol==0.and.ABS(dtm)>tiny.and..not.sgl)then
      ...
      do 501 , k = 1,2
        ...
        if(neta>tiny .and. kstar(k)<15)then
          ...
          if(beta<0.d0)then !PK. following startrack
            beta = 0.125
            if(kstar(k)<=1)then
            ...
            elseif(kstar(k)>=7.and.kstar(k)<=9)then
              if(mass(k)>120.d0)then
                beta = 7.d0
...
501   continue
      ...
      !check for kstar added by PA
      if(neta>tiny .and. kstar(j2)<15)then
      ...
      if(sep<=100000.d0.and.grflag==1)then
      ...
      do 502 , k = 1,2
        ...
        if(kstar(k)==13.and.pulsar>0)then
        ...
        !            if(mass(k)>0.35d0.and.kstar(k)<10.and.
        !     &              menv(k)>0.0d0)then
        ...
        if(htpmb==0)then
          ...
          if(ST_cr<=0.and.mass(k)>0.35d0.and.&
            kstar(k)<10.and.menv(k)>0.0d0)then
          ...
          elseif(ST_cr>0.and.menv(k)>0.d0.and.&
            ((kstar(k)<=1.and.mass(k)>0.35d0.and.&
            mass(k)<=convradcomp(k)).or.&
            (kstar(k)==2.and.teff(k)<=convradcomp(k)).or.&
            (kstar(k)==4.and.teff(k)<=convradcomp(k)).or.&
            ((kstar(k)==3).or.(kstar(k)==5).or.&
            (kstar(k)==6))))then
            ! Ivanova & Taam (2002) method
            if(ospin(k)<=wx) djmb = kw3 * rad(k)**4.0d0 *&
              (ospin(k)/wsun)**3.0d0
        ...
        dspint(k) = 0.d0
        if(((kstar(k)<=9.and.rad(k)>=0.01d0*rol(k)).or.&
          (kstar(k)>=10.and.k==j1.and.kstar(k)<=12))&
          .and.tflag>0)then
          raa2 = (rad(k)/sep)**2
          raa6 = raa2**3
          ! Hut's polynomials.
          ...
          f1 = 1.d0+ecc2*(15.5d0+ecc2*(31.875d0+ecc2*(11.5625d0&
            +ecc2*0.390625d0)))
          if(ST_cr<=0.and.&
            ((kstar(k)==1.and.mass(k)>=1.25d0).or.&
            kstar(k)==4.or.kstar(k)==7))then
            ...
            rg2 = k2str(k)
          elseif(ST_cr>0.and.&
            ((kstar(k)<=1.and.mass(k)>convradcomp(k)).or.&
            (kstar(k)==2.and.teff(k)>convradcomp(k)).or.&
            (kstar(k)==4.and.teff(k)>convradcomp(k)).or.&
            (kstar(k)==9.and.mass(k)>convradcomp(k)).or.&
            (kstar(k)==7).or.(kstar(k)==8)))then
            ! Radiative damping (Zahn, 1977, A&A, 57, 383 and 1975, A&A, 41, 329).
            tc = 1.592d-09*(mass(k)**2.84d0)
            f = 1.9782d+04*SQRT((mass(k)*rad(k)*rad(k))/sep**5)*&
              tc*(1.d0+q(3-k))**(5.d0/6.d0)
          ...
          if(kstar(k)<=6.or.ABS(djt)/jspin(k)>0.1d0)then
            djtt = djtt + djt
          endif
        endif
502   continue
    ...
    elseif(ABS(dtm)>tiny.and.sgl)then
      do 503 , k = kmin,kmax
        ...
        elseif(htpmb==1)then
          if(ST_cr<=0.and.&
            mass(k)>0.35d0.and.kstar(k)<10.and.&
            menv(k)>0.0d0)then
    ...
    do 504 , k = kmin,kmax
      ...
      if(kstar(k)<10)then
        ...
        if(dms(k)>0.01d0*mass(k))then
          ...
          if(k==2) dms(1) = dms(1)*0.01d0*mass(2)/dms(2)
          dms(k) = 0.01d0*mass(k)
          dt = 1.0d+06*dtm
        endif
      endif
...
504 continue
    ...
    do 505 , k = kmin,kmax
      ...
      if((jspin(k)>jspbru.or.&
        (jspin(k)==1.0d-10.and.intpol>0)).and.&
        ABS(dtm)>tiny)then !PDK add check for jspin intpol issues here.
      ...
      ! Update mass
      if(ABS(dms(k))>tiny)then
        mass(k) = mass(k) - dms(k)
        if(kstar(k)<=2.or.kstar(k)==7)then
          m0 = mass0(k)
          mass0(k) = mass(k)
        ...
        if(kstar(k)==13.and.pulsar>0)then
          if(dms(k)<0.d0)then !negative dms is mass gained.
...
505 continue
    if(.not.sgl)then
      ecc1 = ecc1 - delet*dt
      ecc = MAX(ecc1,0.d0)
      if(ecc<1.0d-10) ecc = 0.d0
      if(ecc>=1.d0) goto 135
      jorb = jorb - djorb*dt
    ...
    do 6 , k = kmin,kmax
      ...
      if((kw.ne.kstar(k).and.kstar(k)<=12.and.&
        (kw==13.or.kw==14.or.(kw==15.and.pisn_track(k)==7)))&
        .or.(ABS(merger)>=20.d0))then
        if(formation(k).ne.11) formation(k) = 1
        if(kw==13.and.ecsn>0.d0)then
          if(kstar(k)<=6)then
            if(mass0(k)<=zpars(5))then
          ...
          elseif(formation(k)==11)then
            ...
            formation(k) = 5
          elseif(kstar(k)>=10.or.kstar(k)==12)then
          ...
          elseif(merger<=-20.d0)then
            ...
            if(merger>=200.d0)then!estimate CC SN
              ...
              !                  if(sigma>=200.d0)then!estimate CC SN
              formation(k) = 1
            else
              formation(k) = 5
            endif
          endif
        elseif(kw==13.and.aic>0)then
...
170         u1 = ran3(idum1)
            ...
            s = 0.7d0*s - 0.6d0
            if(s>=0.013d0.or.s<=-1.5d0) goto 170
...
174         u1 = ran3(idum1)
            ...
            s = sqrt(-2.d0*LOG(1.d0-u1))*COS(twopi*u2)
            s = 0.68d0*s + 12.6d0
            if(s<11.5d0.or.s>13.8d0) goto 174
            B_0(k) = 10.d0**s
...
175           u1 = ran3(idum1)
              ...
              !                  s = 0.7d0*s - 0.6d0
              s = 0.5d0*s - 2.25d0
              !                  if(s>=0.013.or.s<=-1.5) goto 173
              !                  if(s>=-2.0457d0.or.s<=-2.53d0) goto 175
              if(s>=-1.6457d0.or.s<=-2.53d0) goto 175
...
176           u1 = ran3(idum1)
              ...
              s = SQRT(-2.d0*LOG(1.d0-u1))*COS(twopi*u2)
              s = 0.3d0*s + 8.38d0
              if(s<8.d0.or.s>8.778d0) goto 176
              B_0(k) = 10.d0**s
            endif
          else
            ospin(k) = 2.0d+08
...
508   continue
    ...
    if((isave.and.tphys>=tsave).or.iplot)then
      if(sgl.or.(rad(1)<rol(1).and.rad(2)<rol(2))&
        .or.tphys<tiny)then
        if(B_0(1)==0.d0)then !PK.
          b01_bcm = 0.d0
        elseif(B_0(1)>0.d0.and.B(1)==0.d0)then
    ...
    if(rad(j1)>rol(j1))then
      ...
      if(intpol>0)then
        ...
        tphys0 = tphys
        if(using_METISSE==1 .and. (dtm<=1.0d-10))&
          dtm = max(ABS(dtm),dtmi(j1))
      endif
    endif
    ! Check for collision at periastron.
    pd = sep*(1.d0 - ecc)
    if(pd<(rad(1)+rad(2)).and.intpol==0) goto 130
    ! Go back for the next step or interpolation.
98  continue
    if(tphys>=tphysf.and.intpol==0) goto 140
    if(change)then
      change = .false.
      evolve_type = 2.d0
...
8   dme = 2.08d-03*eddfac*(1.d0/(1.d0 + zpars(11)))*rad(j2)*tb
    ...
    do 13 , k = 1,2
      ...
      if(kstar(k)<=1.or.kstar(k)==7.or.kstar(k)>=10)then
        tkh(k) = tkh(k)*mass(k)
      else
...
13  continue
    ...
    if(qcflag==0)then
      ...
      if(kstar(j1)==0)then
        qc = 0.695
      elseif(kstar(j1)==1)then
        qc = 3.d0
      elseif(kstar(j1)==2)then
        qc = 4.d0
      elseif(kstar(j1)==3)then
        qc = (1.67d0-zpars(7)+2.d0*(massc(j1)/mass(j1))**5)/2.13d0
      elseif(kstar(j1)==4)then
        qc = 3.d0
      elseif(kstar(j1)==5)then
        qc = (1.67d0-zpars(7)+2.d0*(massc(j1)/mass(j1))**5)/2.13d0
      elseif(kstar(j1)==6)then
        qc = (1.67d0-zpars(7)+2.d0*(massc(j1)/mass(j1))**5)/2.13d0
      elseif(kstar(j1)==7)then
        qc = 3.0d0
      elseif(kstar(j1)==8)then
        qc = 0.784d0
      elseif(kstar(j1)==9)then
        qc = 0.784d0
      elseif(kstar(j1)>=10)then
        qc = 0.628
      endif
    elseif(qcflag==1)then
    ...
    if((kstar(j1)<=1.or.kstar(j1)==7).and.q(j1)>qc)then
      ...
      if(kstar(j2)<=1)then
        ...
        if(mass(j2)<0.35d0.or.mass(j2)>1.25d0)then
      ...
      elseif(kstar(j2)<=6)then
        ...
        if(kstar(j2)==2)then
          mass0(j2) = mass(j2)
          if (using_METISSE==1) call set_star_type(j2)
      ...
      elseif(kstar(j2)<=12)then
        ...
        if((kstar(j2)==10.and.mass(j2)<0.05d0).or.&
          (kstar(j2)>=11.and.mass(j2)<0.1d0))then
          kst = kstar(j1)
          mass(j1) = mass(j2) + dm2
          mass(j2) = 0.d0
          if (using_METISSE==1) call set_star_type(j1)
        else
          mass(j2) = mass(j2) + dm2
    ...
    elseif(((kstar(j1)==3.or.kstar(j1)==5.or.kstar(j1)==6.or.&
      kstar(j1)==8.or.kstar(j1)==9)&
      .and.(q(j1)>qc.or.radx(j1)<=radc(j1))).or.&
      (kstar(j1)==2.and.q(j1)>qc).or.&
      (kstar(j1)==4.and.q(j1)>qc))then
      ...
      if(j1==2.and.kcomp2==13.and.kstar(j2)==15.and.&
        kstar(j1)==13)then !PK.
        ! In CE the NS got switched around. Do same to formation.
        formation(j1) = formation(j2)
      endif
      if(j1==1.and.kcomp2==13.and.kstar(j2)==15.and.&
        kstar(j1)==13)then !PK.
        ! In CE the NS got switched around. Do same to formation.
        formation(j1) = formation(j2)
      endif
      ...
      if(binstate==1.d0)then
        sep = 0.d0
        tb = 0.d0
      elseif(binstate==2.d0)then
        sep = -1.d0
        tb = -1.d0
      endif
      if(j1==2.and.kcomp2==13.and.kstar(j2)==15.and.&
      ...
      do 117, k = 1,2
        ...
        if((kw.ne.kstar(k).and.kstar(k)<=12.and.&
          (kw==13.or.kw==14&
          .or.(kw==15.and.pisn_track(k)==7))))then
          dms(k) = mass(k) - mt
          if(formation(k).ne.11) formation(k) = 1
          if(kw==13.and.ecsn>0.d0)then
            if(kstar(k)<=6)then
          ...
          if(mass(3-k)<0.d0)then
            if(kstar(3-k)<0.d0) mt = mt-mass(3-k)
            if(kw==13.and.mt>mxns) kw = 14
            CALL CONCATKSTARS(kstar(j1), kstar(j2), mergertype)
            kstar(k) = kw
            mass(k) = mt
...
117   continue
      ...
      !         if(kstar(1)==15) mass1_bpp = mass0(1)
      !         if(kstar(2)==15) mass2_bpp = mass0(2)
      rrl1 = rad(1)/rol(1)
      rrl2 = rad(2)/rol(2)
      teff1 = 1000.d0*((1130.d0*lumin(1)/&
        (rad(1)**2.d0))**(1.d0/4.d0))
      ...
      tb = (sep/aursun)*SQRT(sep/(aursun*(mass(1)+mass(2))))
      oorb = twopi/tb
    elseif(kstar(j1)>=10.and.kstar(j1)<=12.and.&
      q(j1)>qc)then
      ...
      if(eddfac<10.d0)then
        !           dm2 = MIN(dme*taum/tb,dm1)
        if(wd_mass_lim==1)then
          dm2 = MIN(dme*taum/tb,dm1)
        elseif(wd_mass_lim==0)then
          dm2 = dm1
        endif
        if(dm2<dm1) supedd = .true.
      else
      ...
      if(kstar(j1)==10.and.kstar(j2)==10)then
        ...
        kstar(j2) = 15
        mass(j2) = 0.d0
      elseif(kstar(j1)==10.and.kstar(j2)>10)then
      ...
      elseif(kstar(j2)<=12)then
        mass0(j2) = mass(j2)
        if(kstar(j1)==12.and.kstar(j2)==11)then
          ! Mixture of ONe and CO will result in an ONe product.
          kstar(j2) = 12
        endif
        if(kstar(j1)==11.and.kstar(j2)==11)then !PK.
      ...
      if(kstar(j2)<=11.and.mass(j2)>mch)then
        kstar(j2) = 15
        mass(j2) = 0.d0
      endif
      coel = .true.
    ...
    elseif(kstar(j1)==14)then
      ...
      if(kstar(j1)>=0.and.kstar(j1)<=9)then
        !**
      ...
      if(rad(j1)>10.d0*rol(j1).and.kstar(j1)<=1.and.&
        kstar(j2)<=1)then
        ! Allow the stars to merge with the product in *1.
        CALL CONCATKSTARS(kstar(j1), kstar(j2), mergertype)
        m1ce = mass(j1)
        m2ce = mass(j2)
        if((kstar(1)>=10.and.kstar(1)<=12).and.&
          (kstar(2)>=10.and.kstar(2)<=12))then
          formation(1) = 11
          formation(2) = 11
        endif
...
15    continue
      ...
      if(iter<=loop) dtm = MIN(dtm,dtmi(1),dtmi(2))
      ...
      if(kstar(j2)<=2.or.kstar(j2)==4)then
        if(acc_lim(j2)==-1.or.acc_lim(j2)==-3)then
          dm2 = MIN(1.d0,10.d0*taum/tkh(j2))*dm1
        elseif(acc_lim(j2)==-2.or.acc_lim(j2)==-4)then
          dm2 = MIN(1.d0,taum/tkh(j2))*dm1
        elseif(acc_lim(j2)>=0.d0)then
          dm2 = acc_lim(j2)*dm1
        endif
      elseif(kstar(j2)>=7.and.kstar(j2)<=9)then
        ...
        if(kstar(j1)>=7)then
          ...
          if(acc_lim(j2)<0.d0)then
            dm2 = dm1
          elseif(acc_lim(j2)>=0.d0)then
            dm2 = acc_lim(j2)*dm1
          endif
          dmchk = dm2 - 1.05d0*dms(j2)
          if(dmchk>0.d0.and.dm2/mass(j2)>1.0d-04)then
        ...
        if(dm1/tb<2.71d-07)then
          if(dm1/tb<1.03d-07)then
            ...
            elseif(acc_lim(j2)>=0.d0)then
              ...
              if(dm2<acc_lim(j2)*dm1) supedd = .true.
            endif
            dm22 = epsnov*dm2
          else
          ...
          ! Check for planets or low-mass WDs.
          if((kstar(j2)==10.and.mass(j2)<0.05d0).or.&
            (kstar(j2)>=11.and.mass(j2)<0.5d0))then
            kst = kstar(j2)
            if (using_METISSE==1) call set_star_type(j2)
          else
      ...
      elseif(kstar(j2)==3.or.kstar(j2)==5.or.kstar(j2)==6)then
        ! We have a giant w/ kstar(j2) = 3,5,6
        if(acc_lim(j2)==-1.or.acc_lim(j2)==-2)then
          dm2 = dm1
        elseif(acc_lim(j2)==-3)then
          dm2 = MIN(1.d0,10*taum/tkh(j2))*dm1
        elseif(acc_lim(j2)==-4)then
          dm2 = MIN(1.d0,taum/tkh(j2))*dm1
        elseif(acc_lim(j2)>=0.d0)then
          dm2 = MIN(dm2,acc_lim(j2)*dm1)
        endif
      ...
      if(kst>=10.and.kst<=12)then
        ...
        if(kstar(j1)<=10.and.kst==10.and.mt2>=0.7d0)then
      ...
      if(supedd.or.novae.or.int(gamma)==-2)then
...
602   continue
      ...
      do 603 , k = 1,2
        ...
        dspint(k) = 0.d0
        if(((kstar(k)<=9.and.rad(k)>=0.01d0*rol(k)).or.&
          (kstar(k)>=10.and.k==j1)).and.tflag>0)then
          raa2 = (radx(k)/sep)**2
          raa6 = raa2**3
          f5 = 1.d0+ecc2*(3.d0+ecc2*0.375d0)
          f4 = 1.d0+ecc2*(1.5d0+ecc2*0.125d0)
...
603   continue
      ...
      if(kstar(j1)<=1.or.kstar(j1)==7) mass0(j1) = mass(j1)
      mass(j2) = mass(j2) + dm22 - dms(j2)
      if(kstar(j2)<=1.or.kstar(j2)==7) mass0(j2) = mass(j2)
      if(kstar(j2)==13.and.pulsar>0)then
      ...
      if(kstar(j1)==2.and.mass0(j1)<=zpars(3))then
        m0 = mass0(j1)
        mass0(j1) = mass(j1)
        CALL star(kstar(j1),mass0(j1),mass(j1),tmsnew,tn,tscls,&
          lums,GB,zpars,dtm,j1)
        if(GB(9)<massc(j1))then
          mass0(j1) = m0
        endif
      endif
      if(kstar(j2)==2.and.mass0(j2)<=zpars(3))then
        m0 = mass0(j2)
        mass0(j2) = mass(j2)
        CALL star(kstar(j2),mass0(j2),mass(j2),tmsnew,tn,tscls,&
          lums,GB,zpars,dtm,j2)
        if(GB(9)<massc(j2))then
          mass0(j2) = m0
        endif
      endif
      ecc = ecc - delet
      ...
      if((jorb-djorb)<1.d0) inspiral=.true.
    ...
    if(kstar(j1)<=2.or.kstar(j1)==7)then
      CALL star(kstar(j1),mass0(j1),mass(j1),tmsnew,tn,tscls,&
        lums,GB,zpars,dtm,j1)
      if(kstar(j1)==2)then
        if (using_SSE==1) aj(j1) = tmsnew + (tscls(1) - tmsnew)*&
    ...
    if(kstar(j2)<=2.or.kstar(j2)==7)then
      CALL star(kstar(j2),mass0(j2),mass(j2),tmsnew,tn,tscls,&
        lums,GB,zpars,dtm,j2)
      if(kstar(j2)==2)then
        if (using_SSE==1) aj(j2) = tmsnew + (tscls(1) - tmsnew)*&
          (aj(j2)-tms(j2))/(tbgb(j2) - tms(j2))
      elseif((mass(j2)<0.35d0.or.mass(j2)>1.25d0)&
    ...
    do 90 , k = 1,2
      ...
      if(kw.ne.kstar(k))then
        change = .true.
        if((kw==13.or.kw==14).and.kstar(k)<=12)then
          snova = .true.
        endif
        mass(k) = mt
        if(kw==15)then
...
90  continue
    do 100 , k = 1,2
      ...
      rol(k) = rl(q(k))*sep*(1.d0-ecc)
100 continue
    if(rad(j1)>rol(j1)) radx(j1) = MAX(radc(j1),rol(j1))
    do 110 , k = 1,2
...
110 continue
    ...
    if(tphys>=tphysf.and..not.inspiral) goto 140
    if(change)then
      change = .false.
      evolve_type = 2.0
      rrl1 = rad(1)/rol(1)
    ...
    if(rad(j1)>rol(j1).and..not.snova)then
      ! Test for a contact system
      if(rad(j2)>rol(j2)) goto 130
      iter = iter + 1
      goto 8
    else
      evolve_type = 4.0
...
130 continue
    ...
    if(output) write(*,*)'coal r/rl1 & r/rl2 > 0',tphys,kcomp1,kcomp2,&
      m1ce,m2ce
    if(kstar(j1)>=2.and.kstar(j1)<=9.and.kstar(j1).ne.7)then
      if(j1==2)then
        switchedCE = .true.
      else
        switchedCE = .false.
    ...
    elseif(kstar(j2)>=2.and.kstar(j2)<=9.and.kstar(j2).ne.7)then
      if(j1==1)then
        switchedCE = .true.
      else
        switchedCE = .false.
      ...
      if(output) write(*,*)'coal2:',tphys,kstar(j1),kstar(j2),coel,&
        mass(j1),mass(j2)
      if(j2==2.and.kcomp1==13.and.kstar(j1)==15.and.&
        kstar(j2)==13)then !PK.
        ! In CE the NS got switched around. Do same to formation.
        formation(j2) = formation(j1)
      endif
      if(j2==1.and.kcomp1==13.and.kstar(j1)==15.and.&
        kstar(j2)==13)then !PK.
        ! In CE the NS got switched around. Do same to formation.
        formation(j2) = formation(j1)
      endif
...
135 continue
    ...
    if(kstar(1)==13.and.mergemsp==1.and.&
      notamerger==0)then
      s = (twopi*yearsc)/ospin(1)
      if(s<0.03d0.and.B(1)>0.d0)then
        merge_mem = 1
      endif
    endif
    if(kstar(2)==13.and.mergemsp==1.and.&
      notamerger==0)then
      s = (twopi*yearsc)/ospin(2)
      if(s<0.03d0.and.B(2)>0.d0)then
        merge_mem = 1
      endif
    endif
    if(kstar(1).ne.15.or.kstar(2).ne.15)then
      ...
      if(kstar(1)==13.and.ecc<=1.d0.and.pulsar>0.and.&
        notamerger==0)then
        age = 0.d0
        epoch(1) = tphys
      endif
      if(kstar(2)==13.and.ecc<=1.d0.and.pulsar>0.and.&
        notamerger==0)then
        age = 0.d0
        epoch(2) = tphys
      endif
...
140 continue
    if(com)then
      ...
      !          if(kstar(1)==15.and.bpp(bpp_ind,4)<15.0)then
      !              mass1_bpp = mass0(1)
      !          endif

      !          if(kstar(2)==15.and.bpp(bpp_ind,5)<15.0)then
      !              mass2_bpp = mass0(2)
      !          endif
      ...
      if(coel)then
        ...
        CALL writetab(bpp_ind,tphys,evolve_type,&
          ...
          deltam1_bcm,deltam2_bcm,formation(1),&
          formation(2),binstate,mergertype,z,'bpp')
      elseif(kstar(1)==15.and.kstar(2)==15)then
        ...
        if(iter>=loop) evolve_type = 100.0
        if (using_METISSE==1) then
          call check_error(err)
...
145     continue
    ...
    tphysf = tphys
    if(sgl)then
      if(ecc>=0.d0.and.ecc<=1.d0) ecc = -1.d0
      tb = -1.d0
    endif
    tb = tb*yeardy

    if(bpp_ind>=1000)then
